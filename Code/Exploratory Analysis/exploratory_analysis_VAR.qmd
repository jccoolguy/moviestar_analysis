...
actor_movie_prior <- actors_var |>
  group_by(movie_id) |>
  summarise(
    # PRIOR (career stock entering this movie)
    actor_power_prior_sum = sum(actor_star_power_prior, na.rm = TRUE),
    actor_power_prior_max = max(actor_star_power_prior, na.rm = TRUE),

    # MOMENTUM (recent heat entering this movie)
    actor_momentum_prior_sum = sum(actor_star_power_momentum, na.rm = TRUE),
    actor_momentum_prior_max = max(actor_star_power_momentum, na.rm = TRUE),
...
df <- df |> 
  ...
  mutate(
    actor_power_prior_concentration =
      ifelse(is.finite(actor_power_prior_concentration),
             actor_power_prior_concentration,
             NA_real_)
  )
```
