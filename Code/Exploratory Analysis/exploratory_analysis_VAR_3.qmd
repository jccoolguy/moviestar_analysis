...
# final residuals used for actor VAR allocation
df$resid_actor <- NA_real_

# keep object name baseline_actor (list of year-by-year models)
baseline_actor <- vector("list", length(years_all))
names(baseline_actor) <- as.character(years_all)

for (t in years_all) {

  train <- df |> filter(year < t)
  test  <- df |> filter(year == t)

  if (nrow(test) == 0) next
  if (nrow(train) < 50 || length(unique(train$year)) < 3) next

  # keep only rows with finite inputs (prevents predict() NAs)
  train <- train |>
    filter(
      is.finite(log_profitability),
      is.finite(year),
      is.finite(ip_evergreen_prior),
      is.finite(ip_momentum_prior)
    )
  ...
  # Simplified actor-conditional baseline: IP priors + time trend only
  m <- lm(
    log_profitability ~ ip_evergreen_prior + ip_momentum_prior + ns(year, df = 4),
    data = train
  )
  baseline_actor[[as.character(t)]] <- m

  # out-of-sample prediction for year t
  pred_t <- predict(m, newdata = test)

  # out-of-sample residuals
  df$resid_actor[df$movie_id %in% test$movie_id] <-
    test$log_profitability - pred_t
}
...
```{r}
modeling_data <- list(
  movie_level_df    = df |> filter(year > 1972),
  actor_level_df    = actors_var |> filter(year > 1972),
  ip_level_df      = ip_tbl |> filter(1972)
)
