...
# baseline0 stays the same object name (list of yearly lm models)
baseline0 <- vector("list", length(years_all))
names(baseline0) <- as.character(years_all)

for (t in years_all) {

  if (t < analysis_start_year) next
  ...
  # Keep only rows with finite model inputs
  train <- train |>
    filter(
  ...
  m <- lm(
    log_profitability ~ log_budget + ns(year, df = 4),
    data = train
  )
  baseline0[[as.character(t)]] <- m

  pred_t <- predict(m, newdata = test)

  df$resid0[df$movie_id %in% test$movie_id] <-
    test$log_profitability - pred_t
}

df <- df |> filter(year >= analysis_start_year)

# Optional diagnostics
# mean(is.na(df$resid0))
# df |> filter(is.na(resid0)) |> count(year) |> arrange(desc(n))
