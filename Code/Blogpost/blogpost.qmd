...
Actor Momentum is especially helpful for assessing actors who have on and off periods of commercial success which would be harder to pickup through Actor Stock. An example would be Eddie Murphy:
...
To examine how the influence of IP and actor-related factors changes over time, we use a generalized additive model (GAM). This approach allows each variable's effect to vary smoothly with release year, capturing gradual shifts in audience preferences and industry structure without imposing a fixed parametric form. We also center and scale all IP and actor related factors, this allows for direct comparison across effects.

# Results

After model selection we ended up with a model with year, budget, IP Momentum (smoothed over year) and Actor Momentum (smoothed over year) all being statistically significant. IP Momentum and Actor Momentum have statistically significant effects over time. Let's see what that looks like:
...
An interesting plot to consider is the difference between these two effects over time:
...
Since each production company face finite budgets decisions about how much to allocate to intellectual property versus cast strength are trade-offs. Insight about whether IP strength or cast strength can be invaluable when production budgets are in the hundreds of millions. As displayed in the previous section IP Momentum, a metric representing IP effect, has become a stronger predictior of profitability over time. Studios have certainly noticed as the percent of top 50 budgeted movies associated with an established IP has consistently climbed over time.
...
Studios have responded to this shift: the share of top 50 budgeted films associated with an established IP has riven steadily since the early 2000s (Figure 8).

In addition to IP effects correlating to better profitability, they have been less risky movies to produce.

```{r}
risk_df <- movie_df |>
  group_by(established_ip) |>
  summarise(
    q10 = quantile(log_profitability, 0.10, na.rm = TRUE),
    q50 = quantile(log_profitability, 0.50, na.rm = TRUE),
    q90 = quantile(log_profitability, 0.90, na.rm = TRUE)
  )

ggplot(risk_df, aes(x = established_ip, y = q50)) +
  geom_point(size = 3) +
  geom_errorbar(
