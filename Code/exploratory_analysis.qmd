...
We have 50 movies a year after the late 70s, before then it hovers around the high 40s.
...
Since we have made changes to the movie table I will copy the steps I took into a .R file so it is easily traceable.
...
```{r}
cast |> 
  group_by(billing_order) |> 
  summarise(n = n()) |> 
  ggplot(aes(x = billing_order)) + 
  geom_bar()
```

# Stars missing analysis

```{r}
stars <- stars |> 
  mutate(missing = is.na(actor_id))
head(stars$missing)
```
...
stars |> 
  group_by(year) |> 
  summarise(missing_num = sum(missing)) |> 
  ggplot(aes(x = year, y = missing_num)) +
  geom_line()
```
